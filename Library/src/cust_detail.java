...
class cust_detail extends JFrame {
	...
	cust_detail(String strz) {
		...
		try {
			...
			if (visible == 0) {
				JOptionPane.showMessageDialog((Component) null, strz
						+ " such no Customer Found !  ",
				...
				try {
					...
					if (!t7.getText().equals("")) {
						...
						if (!t7.getText().equals("")) {
							...
							wr1 = new FileWriter("Cust_Details/Cus" + xx
									+ ".bname");
							wr1.write(t7.getText() + "");
							wr1.close();
							...
							for (int i = 1; i < count; i++) {
								try {
									...
									if (book_name.toLowerCase().equals(
											t7.getText().toLowerCase())) {
										wr1 = new FileWriter("Database/" + i
												+ ".id");
